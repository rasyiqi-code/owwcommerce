...
<!-- Setting data produk untuk owwc-single-product-app eksternal -->
<script id="owwc-product-settings-json" type="application/json">
{
    "productType": <?php echo json_encode($product->type); ?>,
    "variations": <?php echo json_encode(array_map(fn($v) => $v->to_array(), $product->variations)); ?>,
    "apiRecUrl": <?php echo json_encode(esc_url_raw( rest_url( 'owwc/v1/products/' . $product->id . '/recommendations' ) )); ?>,
    "apiReviewUrl": <?php echo json_encode(esc_url_raw( rest_url( 'owwc/v1/products/' . $product->id . '/reviews' ) )); ?>
}
</script>
